...
public:
    FrameRecorder(int max_frames, int w, int h);
    ~FrameRecorder() { if(f.is_open()) f.close(); };
    bool process(int vblank, int pixel_en, int pixel);
};

FrameRecorder::FrameRecorder(int max_frames=35, int w=256, int h=240)
    : frame_num(0), max_frames(max_frames), w(w), h(h)
{
...
bool FrameRecorder::process(int vblank, int pixel_en, int pixel)
{
    if (f.is_open())
    {
        ...
        if (vblank)
        {
            ...
            f.close();
            if(frame_num >= max_frames) return false;
        }
    }
    else if (!vblank)
    {
        ...
        fname << "logs/frame" << frame_num << ".ppm";
...
int main(int argc, char** argv, char** env) {
    ...
    while (!Verilated::gotFinish() && stillgoing) {
        ...
        if(top->clk_ppu)
            stillgoing = framerec.process(top->vblank, top->pixel_en, top->pixel);
